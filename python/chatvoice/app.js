...
function addBotItem(text) {
  ...
  appContent.innerHTML += "<div class='calloutbig'><img src='bot.png' width='45px' height='45px' class='circular--square' style='float: left;' /><div class='calloutright'>" + text + "</div><div class='message-from message-from-bot'>Siri Replied at "+ new Date().toLocaleString() +"</div></div>";
...
document.addEventListener("DOMContentLoaded", function(event) {
  displayCurrentTime();
  if (!isChrome()) {
    addError("Hi, i'm a robot");
    return;
  }
  if (!('speechSynthesis' in window)) {
    addError("Your browser doesnt support speech synthesis. ''SMKH  BOT'' wont work.");
    return;
  }
  ...
  addBotItem("Hi! I'm ''Siridatos TALKING ROBOT'' ");
